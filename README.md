...
4. Open Inkscape and navigate to the `Preferences`. On Windows and Linux this is in the `Edit` menu. On Mac this is called `Settings` in the `Inkscape` menu.)
...
8. Restart Inkscape
9. The `Extensions` menu should now contain a submenu called `Shaper Origin`, which gives access to the three extensions.
